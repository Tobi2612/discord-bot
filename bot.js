...
client.on('messageCreate', async message => {
  ...
  if (message.content.startsWith('!')) {
    ...
    if (CMD_NAME === 'cmd') {
      ...
      let test_server_role =
        message.member.roles.cache.has('988454685704847380');
  ...
  if (
    message.content.includes('@932996374562238474') ||
    message.content.includes('<@!932996374562238474>')
  ) {
  ...
  if (message.content.toLowerCase().includes('ok hi')) {
    message.channel.send(
      `https://tenor.com/view/ok-hi-hi-hello-ok-bye-ok-hi-kid-gif-19799968`
    );
  }

  if (message.content === '!live') {
    message.channel.send(
      'Hey @everyone, Jess is now live on https://www.twitch.tv/spaceyflower21 ! Go check it out!!'
    );
  }
  ...
  if (
    ...
    message.content === '!mixtape'
  ) {
    if (
      message.channelId == 801881827916513320 ||
      message.channelId == 803352079519318098 ||
      message.channelId == 933130158595002371 ||
      message.channelId == 1236752798159208609
    ) {
      if (message.content === '!map') {
        const getCurrentMap = async () => {
          let current_map = await getMap();

          let timee = humanizeDuration(
            current_map.br_pubs.current.remainingMins * 60000,
            { delimiter: ' and ', units: ['d', 'h', 'm'] }
          );
          let next_timee = humanizeDuration(
            current_map.br_pubs.next.DurationInMinutes * 60000,
            { delimiter: ' and ', units: ['d', 'h', 'm'] }
          );

          message.channel
            .send(`Current Map: ${current_map.br_pubs.current.map}, ${timee}. 
Next Map: ${current_map.br_pubs.next.map}, ${next_timee}`);
      ...
      if (message.content === '!ranked') {
        const getCurrentMap = async () => {
          ...
          let timee = humanizeDuration(
            current_map.br_ranked.current.remainingMins * 60000,
            { delimiter: ' and ', units: ['d', 'h', 'm'] }
          );
          let next_timee = humanizeDuration(
            current_map.br_ranked.next.DurationInMinutes * 60000,
            { delimiter: ' and ', units: ['d', 'h', 'm'] }
          );

          message.channel
            .send(`Current Map: ${current_map.br_ranked.current.map}, ${timee}. 
Next Map: ${current_map.br_ranked.next.map}, ${next_timee}`);
      ...
      if (message.content === '!mixtape') {
        const getCurrentMap = async () => {
          ...
          let next_timee = humanizeDuration(
            current_map.mixtape.next.DurationInMinutes * 60000,
            { delimiter: ' and ', units: ['d', 'h', 'm'] }
          );

          message.channel
            .send(`Current Event: ${current_map.mixtape.current.eventName} on ${current_map.mixtape.current.map}: ${current_map.mixtape.current.remainingTimer}. 
Next Event: ${current_map.mixtape.next.eventName} on ${current_map.mixtape.next.map}, ${next_timee}`);
    ...
    //mixtape.current.map && mixtape.current.eventName || mixtape.next.map && mixtape.next.eventName
      ...
      message.channel.send(
        `This command only works in the gaming channel :unamused:`
      );
    }
  }
  ...
  if (message.content.startsWith(PREFIX)) {
    ...
    if (CMD_NAME === 'live') {
      if (args.length === 0)
        return message.reply('Please provide a valid twitch channel');
...
const goLive = async () => {
  ...
  if (type.type) {
    if (type.type == 'live') {
      ...
      const streamstart_plus6minutes = new Date(
        stream_start_time.getTime() + 4 * 60000
      );
...
const getMap = async () => {
  try {
    let mapp = await axios.get(
      `https://api.mozambiquehe.re/maprotation?version=5&auth=${process.env.APEX_LEGENDS_API_KEY}`
    );
